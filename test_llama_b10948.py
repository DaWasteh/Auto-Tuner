...
def test_vision_with_dflash2_stays_gated_through_b10948(
    ...
    if blocked:
        with pytest.raises(ValueError, match=rf"b{build}.*since b10896.*b10948"):
...
def test_v41_block_names_b10948(tmp_path):
    profile = match_profile("DeepSeek-V4.1-Flash", _profiles())
    assert "b10948" in profile.runtime_block_reason
    assert "b10930" not in profile.runtime_block_reason
    with pytest.raises(ValueError, match="b10948"):
    ...
    with pytest.raises(ValueError, match="b10948 / conversion-only PR #28696"):
